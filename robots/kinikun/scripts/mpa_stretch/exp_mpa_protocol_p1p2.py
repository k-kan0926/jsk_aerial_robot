#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""
MPA experiment protocol with p1/p2 output (rosbag recording optimized)

Publishes:
  /mpa_cmd          : geometry_msgs/Vector3  (x=p1[MPa], y=p2[MPa], z=0)
  /exp/p1_cmd_MPa   : std_msgs/Float32  (commanded p1 in MPa)
  /exp/p2_cmd_MPa   : std_msgs/Float32  (commanded p2 in MPa)
  /exp/ps_calc_MPa  : std_msgs/Float32  (calculated p_sum for reference)
  /exp/pd_calc_MPa  : std_msgs/Float32  (calculated p_diff for reference)
  /exp/mode         : std_msgs/String   (state indicator; latched)
  /exp/is_busy      : std_msgs/Bool     (latched)
  /exp/tag          : std_msgs/String   (lightweight tag for labeling)

Inputs (sensors for settle check):
  ~theta_topic : std_msgs/Float32 [rad] or [deg] (specify via ~theta_unit)
  ~z_topic     : std_msgs/Float32 [m]

Services:
  /exp/pause, /exp/resume, /exp/skip_point, /exp/estop  (std_srvs/Trigger)

Modes: static_grid / dyn_prbs / dyn_multisine / dyn_chirp / dyn_random

Example usage (Dataset 1: static_low):
rosrun kinikun exp_mpa_protocol_p1p2.py \
  _mode:=static_grid _rate_hz:=200 \
  _p_max_MPa:=0.70 \
  _p_sum_min_MPa:=0.05 _p_sum_max_MPa:=0.75 \
  _grid_sum_pts:=15 _grid_diff_pts:=17 \
  _dwell_sec:=2.0 _settle_min_sec:=1.0 _settle_max_sec:=5.0 \
  _slope_win_sec:=0.30 _settle_eps_theta:=0.15 _settle_eps_z:=0.0015 \
  _static_repeat:=2 _theta_topic:=/theta _z_topic:=/z \
  _theta_unit:=rad _warmup_sec:=3.0

Example (Dataset 3: dyn_prbs_broad):
rosrun kinikun exp_mpa_protocol_p1p2.py \
  _mode:=dyn_prbs _session_sec:=900 _rate_hz:=200 \
  _dyn_ps_bias:=0.55 _dyn_ps_span:=0.30 _dyn_pd_span:=0.65 \
  _prbs_pd_interval_min:=0.08 _prbs_pd_interval_max:=0.25 \
  _prbs_ps_interval_min:=0.40 _prbs_ps_interval_max:=1.20 \
  _rate_ps_max:=3.0 _rate_pd_max:=6.0 \
  _theta_topic:=/theta _z_topic:=/z _theta_unit:=rad _warmup_sec:=2.0
"""

import math, random, signal
from collections import deque

import numpy as np
import rospy
from std_msgs.msg import Float32, String, Bool
from geometry_msgs.msg import Vector3
from std_srvs.srv import Trigger, TriggerResponse

# ---------- Coordinate conversions ----------
def clamp(x, lo, hi): 
    return lo if x < lo else (hi if x > hi else x)

def ps_pd_to_p1p2(ps, pd): 
    """Convert (p_sum, p_diff) to (p1, p2)"""
    return 0.5*(ps + pd), 0.5*(ps - pd)

def p1p2_to_ps_pd(p1, p2): 
    """Convert (p1, p2) to (p_sum, p_diff)"""
    return (p1 + p2), (p1 - p2)

def feasible_clip_p1p2(p1, p2, pmax):
    """Clip p1, p2 to [0, pmax] box constraint"""
    p1 = clamp(p1, 0.0, pmax)
    p2 = clamp(p2, 0.0, pmax)
    return p1, p2

def rate_limit(x, x_prev, rate_max, dt):
    """Apply rate limit: |dx/dt| <= rate_max"""
    if rate_max <= 0: 
        return x
    dx = clamp(x - x_prev, -rate_max*dt, rate_max*dt)
    return x_prev + dx

def slope_of(series, times):
    """Linear regression slope of series vs times"""
    n = len(series)
    if n < 2: 
        return 0.0
    t = np.asarray(times)
    y = np.asarray(series)
    tmean, ymean = t.mean(), y.mean()
    denom = np.sum((t - tmean)**2)
    if denom <= 1e-12:
        dt = max(1e-6, t[-1]-t[0])
        return float((y[-1]-y[0])/dt)
    return float(np.sum((t - tmean)*(y - ymean)) / denom)

# ---------- Main Node ----------
class ExpMPAProtocolP1P2:
    def __init__(self):
        # Core parameters
        self.rate_hz = rospy.get_param("~rate_hz", 200)
        self.dt = 1.0 / float(self.rate_hz)

        # Pressure limits
        self.pmax = float(rospy.get_param("~p_max_MPa", 0.70))
        self.ps_min = float(rospy.get_param("~p_sum_min_MPa", 0.0))
        self.ps_max = float(min(rospy.get_param("~p_sum_max_MPa", 1.40), 2.0*self.pmax))

        # Publishers (p1/p2 primary outputs)
        self.pub_cmd = rospy.Publisher("mpa_cmd", Vector3, queue_size=1)
        self.pub_p1 = rospy.Publisher("exp/p1_cmd_MPa", Float32, queue_size=1)
        self.pub_p2 = rospy.Publisher("exp/p2_cmd_MPa", Float32, queue_size=1)
        # Optional ps/pd for reference/debugging
        self.pub_ps = rospy.Publisher("exp/ps_calc_MPa", Float32, queue_size=1)
        self.pub_pd = rospy.Publisher("exp/pd_calc_MPa", Float32, queue_size=1)
        # Status
        self.pub_mode = rospy.Publisher("exp/mode", String, queue_size=1, latch=True)
        self.pub_busy = rospy.Publisher("exp/is_busy", Bool, queue_size=1, latch=True)
        self.pub_tag = rospy.Publisher("exp/tag", String, queue_size=1)

        # Sensor subscribers
        self.joint_topic = rospy.get_param("~joint_topic", "/kinikun1/joint_states")
        self.joint_index = int(rospy.get_param("~joint_index", 2))  # arm3_joint (index 2)
        self.height_topic = rospy.get_param("~height_topic", "/arm1/pose")
        
        self.theta = 0.0  # Joint angle in radians
        self.z_abs = 0.0  # Absolute height from pose
        self.z0 = None    # Initial height (set on first message)
        self.dz = 0.0     # Relative displacement from z0
        
        # Subscribe to JointState for theta
        from sensor_msgs.msg import JointState
        from geometry_msgs.msg import PoseStamped
        rospy.Subscriber(self.joint_topic, JointState, self._cb_joint_state)
        rospy.Subscriber(self.height_topic, PoseStamped, self._cb_height)

        # Static grid parameters
        self.grid_sum_pts = int(rospy.get_param("~grid_sum_pts", 12))
        self.grid_diff_pts = int(rospy.get_param("~grid_diff_pts", 13))
        self.dwell_sec = float(rospy.get_param("~dwell_sec", 1.0))
        self.settle_min_sec = float(rospy.get_param("~settle_min_sec", 0.5))
        self.settle_max_sec = float(rospy.get_param("~settle_max_sec", 3.0))
        
        # Settle thresholds (only for static mode)
        self.settle_eps_theta = float(rospy.get_param("~settle_eps_theta", 0.003))  # rad/s (~0.17 deg/s)
        self.settle_eps_dz = float(rospy.get_param("~settle_eps_dz", 0.002))  # m/s (2mm/s)
        self.slope_win_sec = float(rospy.get_param("~slope_win_sec", 0.25))
        self.static_repeat = int(rospy.get_param("~static_repeat", 1))
        
        # Enable/disable settle check (auto: True for static, False for dynamic)
        self.check_settle = rospy.get_param("~check_settle", "auto")  # "auto", True, False

        # Dynamic parameters
        self.mode = str(rospy.get_param("~mode", "static_grid"))
        self.session_sec = float(rospy.get_param("~session_sec", 600.0))
        self.ps_bias = float(rospy.get_param("~dyn_ps_bias", 0.5))
        self.ps_span = float(rospy.get_param("~dyn_ps_span", 0.3))
        self.pd_span = float(rospy.get_param("~dyn_pd_span", 0.6))
        self.rate_ps_max = float(rospy.get_param("~rate_ps_max", 2.0))
        self.rate_pd_max = float(rospy.get_param("~rate_pd_max", 4.0))

        # PRBS
        self.prbs_pd_interval_min = float(rospy.get_param("~prbs_pd_interval_min", 0.15))
        self.prbs_pd_interval_max = float(rospy.get_param("~prbs_pd_interval_max", 0.35))
        self.prbs_ps_interval_min = float(rospy.get_param("~prbs_ps_interval_min", 0.6))
        self.prbs_ps_interval_max = float(rospy.get_param("~prbs_ps_interval_max", 1.2))

        # Multisine
        self.ms_pd_freqs = self._parse_freqs(rospy.get_param("~multisine_pd_freqs", "0.3,0.6,1.2,2.4"))
        self.ms_ps_freqs = self._parse_freqs(rospy.get_param("~multisine_ps_freqs", "0.2,0.4"))

        # Chirp
        self.chirp_f_lo = float(rospy.get_param("~chirp_f_lo", 0.2))
        self.chirp_f_hi = float(rospy.get_param("~chirp_f_hi", 5.0))
        self.chirp_two_sided = bool(rospy.get_param("~chirp_two_sided", True))

        # Random walk
        self.random_step_ps = float(rospy.get_param("~random_step_ps", 0.02))
        self.random_step_pd = float(rospy.get_param("~random_step_pd", 0.04))

        self.warmup_sec = float(rospy.get_param("~warmup_sec", 0.0))

        # Internal state (work in ps/pd, output p1/p2)
        self.ps_cmd = clamp(self.ps_bias, self.ps_min, self.ps_max)
        self.pd_cmd = 0.0
        self.ps_prev = self.ps_cmd
        self.pd_prev = self.pd_cmd
        self._next_pd_switch = 0.0
        self._next_ps_switch = 0.0

        # Static sequence
        self.static_seq = None
        self.static_idx = 0
        self.static_phase = "init"
        self.point_start_time = None

        # Settle detection buffers
        self.buf_time = deque()
        self.buf_theta = deque()
        self.buf_dz = deque()
        self.buf_maxlen = max(3, int(self.slope_win_sec / self.dt))
        
        # Auto-determine settle check mode
        if self.check_settle == "auto":
            self.check_settle = (self.mode == "static_grid")
        elif isinstance(self.check_settle, str):
            self.check_settle = (self.check_settle.lower() in ["true", "1", "yes"])
        
        rospy.loginfo(f"[exp_mpa_p1p2] Settle check: {self.check_settle} (mode={self.mode})")

        # Control flags
        self.is_paused = False
        self.is_estop = False

        # Services
        rospy.Service("exp/pause", Trigger, self._srv_pause)
        rospy.Service("exp/resume", Trigger, self._srv_resume)
        rospy.Service("exp/skip_point", Trigger, self._srv_skip)
        rospy.Service("exp/estop", Trigger, self._srv_estop)

        # Signal handler
        signal.signal(signal.SIGINT, self._on_sigint)

        rospy.loginfo(f"[exp_mpa_p1p2] Initialized. mode={self.mode}, rate={self.rate_hz}Hz, pmax={self.pmax}MPa")
        rospy.loginfo(f"[exp_mpa_p1p2] joint_topic={self.joint_topic}, joint_index={self.joint_index}")
        rospy.loginfo(f"[exp_mpa_p1p2] height_topic={self.height_topic}")
        if self.check_settle:
            rospy.loginfo(f"[exp_mpa_p1p2] Settle thresholds: theta={self.settle_eps_theta:.4f} rad/s, dz={self.settle_eps_dz:.4f} m/s")

    # ---- Callbacks ----
    def _cb_joint_state(self, msg):
        """Extract theta from JointState message"""
        try:
            if 0 <= self.joint_index < len(msg.position):
                self.theta = float(msg.position[self.joint_index])
            else:
                rospy.logwarn_once(f"[exp_mpa_p1p2] joint_index={self.joint_index} out of range (len={len(msg.position)})")
        except Exception as e:
            rospy.logwarn_throttle(10.0, f"[exp_mpa_p1p2] JointState callback error: {e}")

    def _cb_height(self, msg):
        """Extract height from PoseStamped message and compute dz"""
        try:
            z_current = float(msg.pose.position.z)
            
            # Initialize z0 on first message
            if self.z0 is None:
                self.z0 = z_current
                rospy.loginfo(f"[exp_mpa_p1p2] Initial height z0 = {self.z0:.6f} m")
            
            self.z_abs = z_current
            self.dz = z_current - self.z0
        except Exception as e:
            rospy.logwarn_throttle(10.0, f"[exp_mpa_p1p2] Height callback error: {e}")

    # ---- Services ----
    def _srv_pause(self, _):
        self.is_paused = True
        return TriggerResponse(success=True, message="Paused")

    def _srv_resume(self, _):
        self.is_paused = False
        return TriggerResponse(success=True, message="Resumed")

    def _srv_skip(self, _):
        if self.mode == "static_grid" and self.static_seq is not None:
            self.static_idx = min(self.static_idx + 1, len(self.static_seq))
            return TriggerResponse(success=True, message="Skipped one static point")
        return TriggerResponse(success=False, message="Skip only for static_grid")

    def _srv_estop(self, _):
        self.is_estop = True
        self._publish_cmd(0.0, 0.0, tag="estop")
        return TriggerResponse(success=True, message="E-STOP engaged")

    def _on_sigint(self, *_):
        try:
            self._publish_cmd(0.0, 0.0, tag="sigint_zero")
        except Exception:
            pass
        raise rospy.ROSInterruptException

    # ---- Helpers ----
    def _parse_freqs(self, obj):
        if isinstance(obj, (list, tuple)):
            return [float(x) for x in obj]
        s = str(obj).strip()
        return [float(x) for x in s.split(",")] if s else []

    def _append_slopes(self, now):
        """Append current sensor values to slope buffers"""
        self.buf_time.append(now)
        self.buf_theta.append(self.theta)
        self.buf_dz.append(self.dz)
        while len(self.buf_time) > self.buf_maxlen:
            self.buf_time.popleft()
            self.buf_theta.popleft()
            self.buf_dz.popleft()

    def _publish_cmd(self, ps, pd, tag=""):
        """
        Main command publisher: works in ps/pd, outputs p1/p2
        Applies rate limits and feasibility constraints
        """
        # Rate limit in ps/pd space
        ps = rate_limit(ps, self.ps_prev, self.rate_ps_max, self.dt)
        pd = rate_limit(pd, self.pd_prev, self.rate_pd_max, self.dt)

        # Convert to p1/p2 and apply box constraints
        p1, p2 = ps_pd_to_p1p2(ps, pd)
        p1, p2 = feasible_clip_p1p2(p1, p2, self.pmax)

        # Back-convert to get feasible ps/pd (for next iteration)
        ps, pd = p1p2_to_ps_pd(p1, p2)

        # Publish p1/p2 (primary outputs for control)
        self.pub_cmd.publish(Vector3(p1*4096/0.9, p2*4096/0.9, 0.0))
        self.pub_p1.publish(Float32(p1))
        self.pub_p2.publish(Float32(p2))
        
        # Publish ps/pd for reference
        self.pub_ps.publish(Float32(ps))
        self.pub_pd.publish(Float32(pd))
        
        if tag:
            self.pub_tag.publish(String(tag))

        self.ps_prev, self.pd_prev = ps, pd
        return p1, p2

    # ---- Static Grid ----
    def _prepare_static(self):
        ps_list = np.linspace(self.ps_min, self.ps_max, max(1, self.grid_sum_pts))
        seq = []
        
        for direction in ["up", "down"]:
            it = ps_list if direction == "up" else ps_list[::-1]
            for ps in it:
                # Feasible pd range given ps and pmax
                pd_max = min(ps, 2.0*self.pmax - ps)
                
                if pd_max < 1e-9:
                    for _ in range(max(1, self.static_repeat)):
                        seq.append((ps, 0.0, direction))
                    continue
                
                pd_list = np.linspace(-pd_max, pd_max, max(1, self.grid_diff_pts))
                for pd in pd_list:
                    for _ in range(max(1, self.static_repeat)):
                        seq.append((ps, pd, direction))
        
        self.static_seq = seq
        self.static_idx = 0
        self.static_phase = "step"
        self.point_start_time = None
        rospy.loginfo(f"[exp_mpa_p1p2] Static sequence prepared: {len(seq)} points")

    def _static_once(self, now):
        if self.static_seq is None:
            self._prepare_static()
        
        if self.static_idx >= len(self.static_seq):
            return False

        ps_tgt, pd_tgt, direction = self.static_seq[self.static_idx]
        tag_base = f"static[{direction}]/idx={self.static_idx}"

        if self.static_phase == "step":
            self.pub_mode.publish(String("static_step"))
            self._publish_cmd(ps_tgt, pd_tgt, tag=f"{tag_base}/step")
            self.point_start_time = now
            self.buf_time.clear()
            self.buf_theta.clear()
            self.buf_dz.clear()
            self.static_phase = "wait"
            return True

        if self.static_phase == "wait":
            self.pub_mode.publish(String("static_wait"))
            self._publish_cmd(ps_tgt, pd_tgt, tag=f"{tag_base}/wait")
            
            if self.check_settle:
                self._append_slopes(now)

            elapsed = now - self.point_start_time
            if elapsed < max(self.dwell_sec, self.settle_min_sec):
                return True

            # Check settling criteria (only if enabled)
            if self.check_settle:
                s_theta = abs(slope_of(list(self.buf_theta), list(self.buf_time)))
                s_dz = abs(slope_of(list(self.buf_dz), list(self.buf_time)))
                
                if (s_theta < self.settle_eps_theta) and (s_dz < self.settle_eps_dz):
                    self.static_phase = "record"
                    return True
                
                if elapsed > self.settle_max_sec:
                    rospy.logwarn(f"[exp_mpa_p1p2] Point {self.static_idx} timeout (theta_slope={s_theta:.5f}, dz_slope={s_dz:.5f})")
                    self.static_phase = "record"
                    return True
            else:
                # No settle check: just wait for dwell_sec
                if elapsed >= self.dwell_sec:
                    self.static_phase = "record"
                    return True
            
            return True

        if self.static_phase == "record":
            self.pub_mode.publish(String("static_record"))
            self._publish_cmd(ps_tgt, pd_tgt, tag=f"{tag_base}/record")
            self.static_idx += 1
            self.static_phase = "step"
            return True

        return True

    # ---- Dynamic Excitation ----
    def _dyn_prbs(self, t):
        if t >= self._next_ps_switch:
            self.ps_cmd = clamp(
                self.ps_bias + random.choice([-1, 1]) * self.ps_span,
                self.ps_min, self.ps_max
            )
            self._next_ps_switch = t + random.uniform(
                self.prbs_ps_interval_min, self.prbs_ps_interval_max
            )
        
        if t >= self._next_pd_switch:
            self.pd_cmd = random.choice([-1, 1]) * self.pd_span
            self._next_pd_switch = t + random.uniform(
                self.prbs_pd_interval_min, self.prbs_pd_interval_max
            )
        
        return self.ps_cmd, self.pd_cmd

    def _dyn_multisine(self, t):
        ps = self.ps_bias
        for i, f in enumerate(self.ms_ps_freqs):
            ps += (self.ps_span / max(1, len(self.ms_ps_freqs))) * \
                  math.sin(2*math.pi*f*t + i*0.7)
        
        pd = 0.0
        for i, f in enumerate(self.ms_pd_freqs):
            pd += (self.pd_span / max(1, len(self.ms_pd_freqs))) * \
                  math.sin(2*math.pi*f*t + i*1.1)
        
        return ps, pd

    def _dyn_chirp(self, t, T):
        tau = (t % T) / T
        if self.chirp_two_sided and int(t / T) % 2 == 1:
            tau = 1.0 - tau
        
        f = self.chirp_f_lo * (self.chirp_f_hi / self.chirp_f_lo)**tau
        pd = self.pd_span * math.sin(2*math.pi*f*t)
        ps = self.ps_bias + 0.5*self.ps_span * math.sin(2*math.pi*0.3*t)
        
        return ps, pd

    def _dyn_random(self, _t):
        self.ps_cmd = clamp(
            self.ps_cmd + random.uniform(-self.random_step_ps, self.random_step_ps),
            self.ps_min, self.ps_max
        )
        self.pd_cmd = self.pd_cmd + random.uniform(-self.random_step_pd, self.random_step_pd)
        
        return self.ps_cmd, self.pd_cmd

    # ---- Main Loop ----
    def spin(self):
        r = rospy.Rate(self.rate_hz)
        self.pub_busy.publish(Bool(True))
        self.pub_mode.publish(String(self.mode))
        
        rospy.loginfo(f"[exp_mpa_p1p2] Starting mode={self.mode}, rate={self.rate_hz}Hz")

        # Warmup phase
        start = rospy.Time.now().to_sec()
        if self.warmup_sec > 1e-6:
            rospy.loginfo(f"[exp_mpa_p1p2] Warmup for {self.warmup_sec:.1f}s")
            while not rospy.is_shutdown() and \
                  rospy.Time.now().to_sec() - start < self.warmup_sec:
                if self.is_estop:
                    break
                if not self.is_paused:
                    self._publish_cmd(self.ps_bias, 0.0, tag="warmup")
                r.sleep()

        # Prepare static sequence if needed
        if self.mode == "static_grid":
            self._prepare_static()

        # Main session loop
        sess_start = rospy.Time.now().to_sec()
        while not rospy.is_shutdown():
            now = rospy.Time.now().to_sec()
            t = now - sess_start

            # E-stop check
            if self.is_estop:
                self._publish_cmd(0.0, 0.0, tag="estop_hold")
                self.pub_mode.publish(String("estop"))
                r.sleep()
                continue

            # Pause check
            if self.is_paused:
                self._publish_cmd(self.ps_prev, self.pd_prev, tag="pause_hold")
                self.pub_mode.publish(String("paused"))
                r.sleep()
                continue

            # Mode dispatch
            if self.mode == "static_grid":
                cont = self._static_once(now)
                if not cont:
                    rospy.loginfo("[exp_mpa_p1p2] Static sequence finished")
                    break
            
            else:
                # Dynamic modes
                if t >= self.session_sec:
                    rospy.loginfo(f"[exp_mpa_p1p2] Dynamic session finished ({self.mode})")
                    break

                if self.mode == "dyn_prbs":
                    ps_tgt, pd_tgt = self._dyn_prbs(t)
                elif self.mode == "dyn_multisine":
                    ps_tgt, pd_tgt = self._dyn_multisine(t)
                elif self.mode == "dyn_chirp":
                    ps_tgt, pd_tgt = self._dyn_chirp(t, max(1.0, self.session_sec))
                elif self.mode == "dyn_random":
                    ps_tgt, pd_tgt = self._dyn_random(t)
                else:
                    rospy.logwarn(f"[exp_mpa_p1p2] Unknown mode={self.mode}, using dyn_prbs")
                    ps_tgt, pd_tgt = self._dyn_prbs(t)

                self._publish_cmd(ps_tgt, pd_tgt, tag="dyn")

            r.sleep()

        # Graceful shutdown
        self._publish_cmd(0.0, 0.0, tag="end_zero")
        self.pub_busy.publish(Bool(False))
        self.pub_mode.publish(String("end"))
        rospy.loginfo("[exp_mpa_p1p2] Experiment complete")


# ---- Entry Point ----
if __name__ == "__main__":
    rospy.init_node("exp_mpa_protocol_p1p2")
    node = ExpMPAProtocolP1P2()
    try:
        node.spin()
    except rospy.ROSInterruptException:
        pass