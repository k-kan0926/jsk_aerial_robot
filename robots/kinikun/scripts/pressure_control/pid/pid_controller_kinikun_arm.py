...
class RelativePoseController:
    def __init__(self):
        ...
        body_sub = message_filters.Subscriber('/body/pose', PoseStamped)
        arm_sub = message_filters.Subscriber('/arm/pose', PoseStamped)

        self.ts = message_filters.ApproximateTimeSynchronizer([body_sub, arm_sub], 10, 0.05)
        self.ts.registerCallback(self.pose_callback)

        rospy.loginfo("Relative Pose PID Controller Started.")

    def reconfigure_cb(self, config, level):
        self.pid_pitch.kp = config.pitch_kp
        self.pid_pitch.ki = config.pitch_ki
        self.pid_pitch.kd = config.pitch_kd
        
        self.pid_yaw.kp = config.yaw_kp
        self.pid_yaw.ki = config.yaw_ki
        self.pid_yaw.kd = config.yaw_kd

        self.target_pitch = math.radians(config.target_pitch)
        self.target_yaw = math.radians(config.target_yaw)

        self.pressure_base = config.pressure_base
        self.dac_scale = config.dac_scale

        rospy.loginfo(f"Config Updated: Tgt P={config.target_pitch:.1f}deg, Y={config.target_yaw:.1f}deg")
        return config
    ...
    def pose_callback(self, body_msg, arm_msg):
        ...
        quat_b = [body_msg.pose.orientation.x, body_msg.pose.orientation.y, body_msg.pose.orientation.z, body_msg.pose.orientation.w]
        ...
        quat_a = [arm_msg.pose.orientation.x, arm_msg.pose.orientation.y, arm_msg.pose.orientation.z, arm_msg.pose.orientation.w]
        ...
        err_pitch = normalize_angle(self.target_pitch - p)
        ...
        p1 = clamp(self.pressure_base + u_pitch, 0.0, 0.8)
        p2 = clamp(self.pressure_base - u_pitch, 0.0, 0.8)
        p3 = clamp(self.pressure_base + u_yaw, 0.0, 0.8)
        p4 = clamp(self.pressure_base - u_yaw, 0.0, 0.8)
        ...
        if rospy.get_time() % 1.0 < 0.1:
            rospy.loginfo(f"Pitch: Tgt={self.target_pitch:.2f} Cur={p:.2f} u={u_pitch:.2f} | Yaw: Tgt={self.target_yaw:.2f} Cur={y:.2f} u={u_yaw:.2f}")
