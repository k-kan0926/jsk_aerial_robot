...
- 安全監視機構
- 非同期ロギング
- 拮抗プリロードフェーズによる安定した初期化
- 【NEW】0度近傍での剛性コスト（張力和コスト）による圧力安定化
"""
...
class ControlSystem:
    ...
    def __init__(self, name, model, meta, device, dt, mppi_params, 
        ...
        # NEW: 剛性コスト関連パラメータ
        self.w_stiff = mppi_params.get('w_stiff', 0.0)  # 張力和コスト重み
        self.theta_rest_range = mppi_params.get('theta_rest_range', 0.0)  # [rad]
        self.p_rest = mppi_params.get('p_rest', 0.0)  # 片側の基準圧力 [MPa]
        self.w_tracking_rest = mppi_params.get('w_tracking_rest', self.w_tracking * 0.3)
        self.theta_deadband = mppi_params.get('theta_deadband', math.radians(1.0))  # 1deg デッドゾーン
    ...
    def update_theta_preload(self, theta_1, theta_2):
        """プリロード中の角度更新（履歴も更新）"""
        diff = abs(theta_1 - theta_2)
        with self.lock:
            if diff > self.theta_diff_threshold:
    ...
    def update_history_with_preload(self, preload_pressure):
        """プリロード中の履歴更新"""
        with self.lock:
            # 圧力履歴を更新
            self.hist_p1_cmd.appendleft(preload_pressure)
            self.hist_p2_cmd.appendleft(preload_pressure)
            # 定常状態なので圧力変化率は0
            self.hist_dp1_dt.appendleft(0.0)
            self.hist_dp2_dt.appendleft(0.0)
    ...
    def cost_function(self, theta, theta_ref, p1, p2, p1_prev, p2_prev,
        ...
        # --- Tracking コスト（状態依存） ---
        if abs(theta_ref) < self.theta_rest_range:
            # 0° 近傍：小さい誤差は無視（デッドゾーン）、それ以上は弱めの tracking
            if err_abs < self.theta_deadband:
                tracking_cost = 0.0
            else:
                # デッドゾーン外側の誤差だけ重み付け
                e_eff = err_abs - self.theta_deadband
                tracking_cost = self.w_tracking_rest * (e_eff ** 2)
        else:
            # 通常領域：従来通りの tracking
            tracking_cost = self.w_tracking * (err ** 2)

        cost = tracking_cost

        # 終端で少し重く（必要ならこちらも同様に状態依存にしても良い）
        if k == H - 1:
            cost += 0.5 * tracking_cost
        ...
        # NEW: 0度近傍での剛性（張力和）コスト
        # |theta_ref| が theta_rest_range 未満のとき、
        #   s = p1 + p2 が s_rest = 2 * p_rest から外れるとペナルティ
        if self.w_stiff > 0.0 and self.theta_rest_range > 0.0:
            if abs(theta_ref) < self.theta_rest_range:
                s = p1 + p2
                s_rest = 2.0 * self.p_rest
                cost += self.w_stiff * (s - s_rest) ** 2
...
class NARX_MPPI_Controller:
    ...
    def __init__(self):
        ...
        # Preload parameters
        self.preload_duration = float(rospy.get_param("~preload_duration", 20.0))
        self.preload_pressure = float(rospy.get_param("~preload_pressure", 0.20))

        # 剛性コスト用の p_rest はデフォルトで preload と同じにする
        p_rest_default = self.preload_pressure
        p_rest = float(rospy.get_param("~p_rest", p_rest_default))
        ...
        mppi_params = {
            ...
            # NEW: 剛性コスト関連
            # 例: w_stiff=5.0, theta_rest_range=10deg≈0.17rad
            'w_stiff': float(rospy.get_param("~w_stiff", 5.0)),
            'theta_rest_range': float(rospy.get_param("~theta_rest_range", 0.17)),
            'p_rest': p_rest
        }
        ...
        rospy.loginfo(f"  Preload: {self.preload_duration}s @ {self.preload_pressure} MPa")
        rospy.loginfo(f"  Stiffness cost: w_stiff={mppi_params['w_stiff']}, "
                      f"theta_rest_range={mppi_params['theta_rest_range']} rad, "
                      f"p_rest={mppi_params['p_rest']} MPa")
    ...
    def spin(self):
        ...
        # Phase 1: 拮抗プリロードフェーズ
        # ===========================================
        self.pub_status.publish(String("preloading"))
        preload_start = rospy.get_time()
        
        rospy.loginfo(f"[MPPI] ========================================")
        rospy.loginfo(f"[MPPI] === Phase 1: Antagonistic Preload ===")
        rospy.loginfo(f"[MPPI] ========================================")
        rospy.loginfo(f"[MPPI]   Duration: {self.preload_duration}s")
        rospy.loginfo(f"[MPPI]   Antagonistic pressure: {self.preload_pressure} MPa (both sides)")
        rospy.loginfo(f"[MPPI]   Purpose: Fill history with real data + ensure stiffness")
        
        preload_count = 0
        while not rospy.is_shutdown():
            elapsed = rospy.get_time() - preload_start
            if elapsed > self.preload_duration:
                break
            
            # 拮抗圧力を出力（両方同じ圧力で剛性確保）
            self.publish_cmd(self.preload_pressure, self.preload_pressure, 
                             self.preload_pressure, self.preload_pressure)
            
            # 履歴を現在の状態で更新（制御ループと同じレートで）
            if preload_count % self.frame_skip == 0:
                # 履歴を実データで埋める
                self.system1.update_history_with_preload(self.preload_pressure)
                self.system2.update_history_with_preload(self.preload_pressure)
                ...
                if self.log_path:
                    self.log_buffer.append({
                        't': rospy.get_time(),
                        'phase': 'preload',
                        'theta1': self.system1.theta_rad,
                        'theta_ref1': 0.0,
                        'error1': -self.system1.theta_rad,
                        'p1_cmd': self.preload_pressure,
                        'p2_cmd': self.preload_pressure,
                        'p1_meas': self.system1.p1_meas,
                        'p2_meas': self.system1.p2_meas,
                        'J1_min': 0.0,
                        'J1_mean': 0.0,
                        'theta2': self.system2.theta_rad,
                        'theta_ref2': 0.0,
                        'error2': -self.system2.theta_rad,
                        'p3_cmd': self.preload_pressure,
                        'p4_cmd': self.preload_pressure,
            ...
            # 進捗表示（5秒ごと）
            if preload_count % int(self.rate_hz * 5) == 0:
                hist_len = len(self.system1.hist_theta)
                rospy.loginfo(f"[MPPI] Preload: {elapsed:.1f}/{self.preload_duration}s - "
                              f"θ1={math.degrees(self.system1.theta_rad):.2f}°, "
                              f"θ2={math.degrees(self.system2.theta_rad):.2f}°, "
                              f"hist_len={hist_len}")
        ...
        # プリロード完了時の状態をログ
        rospy.loginfo(f"[MPPI] ========================================")
        rospy.loginfo(f"[MPPI] === Preload Complete ===")
        rospy.loginfo(f"[MPPI] ========================================")
        rospy.loginfo(f"[MPPI]   System1 theta: {math.degrees(self.system1.theta_rad):.2f} deg")
        rospy.loginfo(f"[MPPI]   System2 theta: {math.degrees(self.system2.theta_rad):.2f} deg")
        rospy.loginfo(f"[MPPI]   History filled: {len(self.system1.hist_theta)} samples")
        rospy.loginfo(f"[MPPI]   P1 history: {list(self.system1.hist_p1_cmd)[:5]}... (first 5)")
        
        # 制御系統の初期圧力コマンドを設定（プリロード圧力から開始）
        self.system1.p1_cmd = self.preload_pressure
        self.system1.p2_cmd = self.preload_pressure
        self.system2.p1_cmd = self.preload_pressure
        self.system2.p2_cmd = self.preload_pressure
        
        # 目標角度を0度に設定（スムーズな移行のため）
        self.system1.set_target(0.0)
        self.system2.set_target(0.0)
        ...
        rospy.loginfo(f"[MPPI] ========================================")
        rospy.loginfo(f"[MPPI] === Phase 2: Active Control ===")
        rospy.loginfo(f"[MPPI] ========================================")
        rospy.loginfo(f"[MPPI]   Starting from preload state")
        rospy.loginfo(f"[MPPI]   Initial pressure: {self.preload_pressure} MPa")
